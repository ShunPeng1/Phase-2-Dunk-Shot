class UiGridLayout<T extends Phaser.GameObjects.GameObject> extends Phaser.GameObjects.Container {
    ...
    public createGrid(minColumn: number, count: number, factoryFunction: (scene: Phaser.Scene, x: number, y: number, index: number) => T, isTranspose = false): void {
        const rowCount = Math.ceil(count / minColumn)
        const columnCount = minColumn
        ...
        for (let i = 0; i < rowCount; i++) {
            ...
            for (let j = 0; j < columnCount; j++) {
                const index = isTranspose ? j * rowCount + i : i * columnCount + j
