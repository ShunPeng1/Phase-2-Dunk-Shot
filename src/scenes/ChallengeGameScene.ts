...
import LevelConfiguration from "../managers/win-predicates/LevelConfiguration"
...
class ChallengeGameScene extends DunkShotGameScene {
    ...
    public init(levelCongiguration : LevelConfiguration){
    ...
    protected setupTopBanner(){
        const topBanner = this.add.image(0, 50, AssetManager.MASKS_210_KEY)
        ...
        const challengeText = this.add.text(150, 90, "CHALLENGE", { fontFamily: 'Arial', fontSize: 18, color: '#ffffff' }).setOrigin(0.5)
        challengeText.setScale(1.1, 1)
        challengeText.setScrollFactor(0)
        challengeText.setDepth(11)

        const numberText = this.add.text(150, 110, this.levelConfiguration.level.toString(), { fontFamily: 'Arial', fontSize: 18, color: '#ffffff' }).setOrigin(0.5)

        numberText.setScrollFactor(0)
        numberText.setDepth(11)


        const icon = this.add.image(265, 100, AssetManager.MASKS_111_KEY)
        icon.setScale(0.5, 0.7)
        icon.setScrollFactor(0)
        icon.setDepth(11)

        const scoreText = this.add.text(315, 100, "0/"+ + this.challengeConfiguration.getGoalPredicate().getGoalValue().toString(), { fontFamily: 'Arial', fontSize: 24, color: '#ffffff' }).setOrigin(0.5)
    ...
    protected setupHoopCountText() : void {
        const hoopCountText = this.add.text(550, 160, "0/"+ (this.orderHoops.length -1) + " Baskets", { fontFamily: 'Arial', fontSize: 24, color: '#939596' }).setOrigin(1)
        hoopCountText.setScrollFactor(0)
        console.log("setupHoopCountText")
        let currentCount = 0
        this.ballInteraction.on(BallInteraction.ENTER_NEXT_HOOP_EVENT, (_enterHoop: BasketballHoop, _lastHoop : BasketballHoop, _perfectCount: number, _bounceCount: number, _isBounceWall: boolean, _isBounceRing: boolean) => {
            currentCount++
    ...
    protected setupHoops(): void {
        ...
        const offsetY = this.ballSpawnPlace.y - startPosition.y
        ...
        const orderHoops : BasketballHoop[] = []
        ...
        this.ballInteraction.on(BallInteraction.ENTER_NEXT_HOOP_EVENT, (_enterHoop: BasketballHoop, lastHoop : BasketballHoop, _perfectCount: number, _bounceCount: number, _isBounceWall: boolean, _isBounceRing: boolean) => {
            if (lastHoop) {
                lastHoop.destroy()
            }

            const nextHoop = orderHoops[currentHoopIndex + 1]
    ...
    protected setupGameStateManager(): void {
        const gameStateManager = new ChallengeGameStateManager(this, this.challengeConfiguration)
        ...
        this.ballInteraction.on(BallInteraction.ENTER_GOAL_HOOP_EVENT, (_enterHoop: BasketballHoop) => {
