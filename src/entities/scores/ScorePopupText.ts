...
class ScorePopupText {
    ...
    private showScorePopupText(
        ...
        // Add Score Text
        if (addScore > 0) {
            const addScoreText = this.scene.add
                .text(position.x, position.y, `+${addScore}`, textStyle)
                .setOrigin(0.5)
        ...
        // Bounce Count Text
        if (bounceCount > 0) {
            const bounceTextMessage = 'Bounce ' + (bounceCount > 1 ? `x${bounceCount}` : '')
            const bounceText = this.scene.add
                .text(position.x, position.y - height, bounceTextMessage, textStyle)
                .setOrigin(0.5)
        ...
        // Prefect Count Text
        if (prefectCount > 0) {
            const prefectTextMessage = 'Perfect ' + (prefectCount > 1 ? `x${prefectCount}` : '')
            const prefectText = this.scene.add
                .text(position.x, position.y - height, prefectTextMessage, textStyle)
                .setOrigin(0.5)
            this.animateText(prefectText, delay) // Further delay this animation
        }
    }

    private animateText(text: Phaser.GameObjects.Text, delay = 0): void {
        text.setAlpha(0)
