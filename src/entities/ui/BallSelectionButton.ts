...
class BallSelectionButton extends UiImageButton {
    ...
    private createCostBanner(cost: number): void {
        ...
        const panel = this.scene.add.image(0, this.height / 2, AssetManager.MASKS_50_KEY)
        ...
        const starImage = this.scene.add.image(-40, this.height / 2 - 5, AssetManager.UI_19_KEY)
    ...
    private tryBuy(): void {
        const star = InventoryManager.getInstance().getItemAsNumber(AssetManager.GOLDEN_STAR_INVENTORY_KEY)
