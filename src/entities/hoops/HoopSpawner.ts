...
class HoopSpawner {
    ...
    public spawnRandomHoop(currentPosition : Phaser.Math.Vector2): BasketballHoop {
        ...
        const hoopSpawnInfo = this.hoopSpawnSet.getRandomHoopSpawnInfo()
    ...
    public spawnRandomCollectible(hoop: BasketballHoop): Collectible | null {
        const collectibleSpawnInfo = this.hoopSpawnSet.getRandomCollectibleSpawnInfo()

        if (collectibleSpawnInfo) {
            const worldPosition = hoop.getInternalHoopWorldPosition()

            let x = worldPosition.x
            let y = worldPosition.y

            const hoopRotation = hoop.getRotation()
    ...
    public spawnNextHoop(currentHoop : BasketballHoop) : BasketballHoop{
        const matrix = currentHoop.getWorldTransformMatrix()
        ...
        const nextHoop = this.spawnRandomHoop(worldPosition)
        this.spawnRandomCollectible(nextHoop)

        const nextHoopWorldPosition = nextHoop.getWorldTransformMatrix() 
    
        this.spawnRandomObstacle(currentHoop, nextHoop, worldPosition.x < nextHoopWorldPosition.tx)
    ...
    public spawnRandomObstacle(lastHoop: BasketballHoop, currentHoop : BasketballHoop, isFacingLeft: boolean): ObstacleBoundaryImage | null{
        ...
        const obstacleSpawnInfo = this.hoopSpawnSet.getRandomObstacleSpawnInfo()

        if (obstacleSpawnInfo) {
            const worldPosition = currentHoop.getInternalHoopWorldPosition()
            ...
            const obstacle = this.obstacleFactory.createObstacle(obstacleSpawnInfo.obstacleType, x, y)
